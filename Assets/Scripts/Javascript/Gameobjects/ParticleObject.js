...
function ParticlesSystem() {
	...
	this.Start = function() {
		if (!this.started) {
			...
			this.emitters.push( new Emitter(this.Transform.position, new Vector(2,2), 10,10, 2000000) );
			this.fields.push( new Field(new Vector(this.Transform.position.x-100,this.Transform.position.y+30),50) );


			console.log(Application.LoadedScene.GameObjects[0].fields[0]);
			this.started = true;
...
/*				if(this.emitters[emitter].position.x >= 600){
					console.log("depasse");
					this.vel = .5;
				}
				else if (this.emitters[emitter].position.x <= 100){
					this.vel = -0.5;
				}

				console.log(this.vel);
				this.emitters[emitter].position.x += this.vel;*/

/*				if(this.emitters[emitter].position.y < 0 || this.emitters[emitter].position.y > canvas.height){
					this.emitters[emitter].position.y -= 2;
				}*/

				//this.emitters[emitter].position.x += 2;
				//this.emitters[emitter].position.y += 2;
